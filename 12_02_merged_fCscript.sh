...
output_folder_main="/mnt/groupMansuy/kerem/tasks/longrna/exp/ee_fig3/preprocess_01/_oldbranch/12_02_merged_featureCounts_umidedup_alig_star_sortmerna"
...
featureCounts -T $num_threads -t exon -g gene_id -a ${genome_dir}/GCF_000001635.27_GRCm39_genomic.gtf \
  -o "${output_folder0}/fig3_counts_gtfD_s00_sortmerna.txt" "${bam_files[@]}" > "${log_dir0}/fig3_counts_gtfD_s00_sortmerna.log" 2>&1

# Run featureCounts for all BAM files
featureCounts -T $num_threads -s 2 -t exon -g gene_id -a ${genome_dir}/GCF_000001635.27_GRCm39_genomic.gtf \
  -o "${output_folder2}/fig3_counts_gtfD_s02_sortmerna.txt" $(ls $reads_dir/*.bam) > "${log_dir2}/fig3_counts_gtfD_s02_sortmerna.log" 2>&1
