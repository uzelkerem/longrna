...
for min_overlap in {2..10}; do
  featureCounts -T $num_threads -s 2 -t exon -g gene_id --minOverlap $min_overlap -a ${genome_dir}/GCF_000001635.27_GRCm39_genomic.gtf \
    -o "${output_folder0}/all_counts_s02_minoverlap${min_overlap}.txt" $(ls $reads_dir/*.bam) > "${log_dir0}/featureCounts_s02_minoverlap${min_overlap}.log" 2>&1
done