...
reads_dir="/mnt/groupMansuy/kerem/tasks/longrna/exp/ee_fig3/preprocess_01/09_01_umidedup_alig_star_trim"


output_folder_main="/mnt/groupMansuy/kerem/tasks/longrna/exp/ee_fig3/preprocess_01/12_01_featCounts_umidedup_alig_star_trim"
...
featureCounts -T $num_threads -t exon -g gene_id -a ${genome_dir}/Mus_musculus.GRCm39.109.CHRonly.gtf \
  -o "${output_folder0}/fig3_counts_gtfE_s00.txt" "${bam_files[@]}" > "${log_dir0}/fig3_counts_gtfE_s00.log" 2>&1

# Run featureCounts for all BAM files
featureCounts -T $num_threads -s 2 -t exon -g gene_id -a ${genome_dir}/Mus_musculus.GRCm39.109.CHRonly.gtf \
  -o "${output_folder2}/fig3_counts_gtfE_s02.txt" $(ls $reads_dir/*.bam) > "${log_dir2}/fig3_counts_gtfE_s02.log" 2>&1
