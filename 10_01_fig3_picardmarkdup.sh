...
# Loop over each sample for reads before umidedup # aka loop1
for file in ${reads_dir1}/*.bam; do
  # Get sample name
  ...
  picard MarkDuplicates \
    I= "${file}" \
    O= "${output_folder1}/${sample}_marked_duplicates.bam" \
    M= "${output_folder1}/${sample}_marked_duplicates_metrics.txt" \
    > "${log_dir1}/${sample}_mark_duplicates.log" 2>&1

  # Generate index for the deduplicated BAM file
  samtools index "${output_folder1}/${sample}_marked_duplicates.bam" 
done
